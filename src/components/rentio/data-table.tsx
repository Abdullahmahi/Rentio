...
export function DataTable<T>({ columns, data, getRowId, searchValue, onRowClick, loading = false, pageSize = 10, emptyMessage }: DataTableProps<T>) {
          ...
          <table className="w-full border-collapse text-sm">
            ...
            <tbody>
              {loading ? Array.from({ length: 5 }).map((_, rowIndex) => (
                ...
                <tr key={getRowId(row)} onClick={() => onRowClick?.(row)} className={cn("h-10 border-b border-border last:border-b-0", onRowClick && "cursor-pointer hover:bg-muted/60")}>
