...
export function DataTable<T>({
  ...
  const allOnPageSelected =
    eligible.length > 0 && eligible.every((row) => selected.has(getRowId(row)));
    ...
    <div className="space-y-3">
      ...
      <div className="overflow-hidden rounded-lg border border-border bg-surface shadow-subtle">
                ...
                {columns.map((column) => {
                  ...
                  return (
                    <th
                      key={column.key}
                      className={cn(
                        "h-10 whitespace-nowrap border-b border-border px-4",
        ...
        {!loading && rows.length === 0 ? (
          <EmptyState
            message={emptyMessage ?? t("empty.table")}
            description={t("empty.tableDescription")}
          />
        ) : null}
      </div>
      <div className="grid grid-cols-[minmax(0,1fr)_auto] items-center gap-3 text-sm text-muted-foreground">
        <span className="min-w-0 truncate">
          {t("table.page", { current: safePage, total: totalPages })}
        </span>
