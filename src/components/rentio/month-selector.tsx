...
export function MonthSelector({ period, onChange }: { period: string; onChange: (period: string) => void }) {
  ...
  return (
    <div className="flex items-center gap-1 rounded-lg border border-border bg-surface p-1">
      <Button size="icon" variant="ghost" aria-label={t("month.previous")} onClick={() => onChange(shiftPeriod(period, -1))}>
        <ChevronLeft className="size-4" />
      </Button>
      <span className="numeric min-w-44 text-center text-sm font-semibold">{formatPeriod(period, i18n.language)}</span>
      <Button size="icon" variant="ghost" aria-label={t("month.next")} onClick={() => onChange(shiftPeriod(period, 1))}>
