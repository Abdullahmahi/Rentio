...
import {
  IMPORT_SCHEMAS, autoMapColumns, summarize, templateFor, validateRows,
  type ImportKind, type ValidatedRow, type ValidationContext,
} from "@/lib/import";
...
function Importer({ kind }: { kind: ImportKind }) {
  ...
  const context = useMemo<ValidationContext>(() => {
    ...
    const propertyById = new Map((portfolio.data?.properties ?? []).map((row) => [row.id, row.name]));
    return {
      propertyNames: new Set((portfolio.data?.properties ?? []).map((row) => lower(row.name))),
      unitKeys: new Set((portfolio.data?.units ?? []).map(
        (unit) => `${lower(propertyById.get(unit.property_id))}|${lower(unit.unit_number)}`,
      )),
      tenantEmails: new Set((portfolio.data?.tenants ?? []).map((row) => lower(row.email)).filter(Boolean)),
      tenantPhones: new Set((portfolio.data?.tenants ?? []).map((row) => lower(row.phone)).filter(Boolean)),
      leaseKeys: new Set((portfolio.data?.leases ?? []).map((lease) => {
  ...
  const commit = useToastMutation({
    mutationFn: async () => {
      ...
      const propertyByName = new Map(
        (portfolio.data?.properties ?? []).map((row) => [row.name.trim().toLocaleLowerCase(), row.id]),
      );
      let inserted = 0;
      let updated = 0;
      ...
      for (const row of importable) {
        ...
        if (kind === "units") {
          const propertyId = propertyByName.get(String(value["property"] ?? "").trim().toLocaleLowerCase());
        ...
        if (kind === "tenants") {
          const email = String(value["email"] ?? "").trim().toLocaleLowerCase();
          const phone = String(value["phone"] ?? "").trim();
          const existing = (portfolio.data?.tenants ?? []).find(
            (tenant) => (email && tenant.email?.toLocaleLowerCase() === email) || (phone && tenant.phone === phone),
          );
          const payload = {
        ...
        if (kind === "leases") {
          ...
          const unit = (portfolio.data?.units ?? []).find(
            (candidate) => candidate.property_id === propertyId
              && candidate.unit_number.toLocaleLowerCase() === String(value["unit_number"]).toLocaleLowerCase(),
          );
          const needle = String(value["tenant"] ?? "").trim().toLocaleLowerCase();
          const tenant = (portfolio.data?.tenants ?? []).find(
            (candidate) => candidate.email?.toLocaleLowerCase() === needle || candidate.phone?.toLocaleLowerCase() === needle,
          );
          ...
          if (existing) {
            ...
            const { data: lease, error } = await supabase.from("leases").insert(payload).select("id").single();
            if (error) throw error;
            await supabase.from("lease_tenants").insert({ lease_id: lease.id, tenant_id: tenant.id, role: "primary" });
            await supabase.from("units").update({ status: "ocupada" }).eq("id", unit.id);
          }
        }

        if (row.isUpdate) updated += 1; else inserted += 1;
      }
    ...
    onSuccess: (result) => { setSummary(result); setStep(5); },
  });

  const reset = () => {
    setStep(1); setHeaders([]); setRows([]); setMapping({}); setSummary(null); setFileError(null);
  };

  const stepTitles = ["import.steps.template", "import.steps.upload", "import.steps.mapping", "import.steps.preview", "import.steps.done"];
    ...
    <div className="space-y-5">
        ...
        {stepTitles.map((key, index) => (
          <li key={key} className="flex items-center gap-2">
            <span className={cn(
              ...
              index + 1 < step ? "border-primary bg-primary text-primary-foreground"
                : index + 1 === step ? "border-primary text-primary" : "border-border text-muted-foreground",
            )}>
              {index + 1 < step ? <CheckCircle2 className="size-3.5" /> : index + 1}
            </span>
            <span className={index + 1 === step ? "text-foreground" : "text-muted-foreground"}>{t(key)}</span>
          </li>
        ))}
      </ol>
        ...
        <section className="rounded-lg border border-border bg-surface p-5 shadow-subtle">
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            <Button variant="outline" onClick={downloadTemplate}>
              <Download className="size-4" />{t("import.downloadTemplate")}
            </Button>
      ...
      {step === 2 ? (
        <section className="space-y-3">
          ...
          <input id={`file-${kind}`} type="file" accept=".csv,text/csv" className="sr-only"
            onChange={(event) => void readFile(event.target.files?.[0])} />
          {fileError ? (
            <p role="alert" className="rounded-lg border border-danger/25 bg-danger/10 px-3 py-2 text-sm text-danger">{fileError}</p>
          ) : null}
          <Button variant="ghost" onClick={() => setStep(1)}>{t("contracts.previousStep")}</Button>
        </section>
      ) : null}
      ...
      {step === 3 ? (
        <section className="space-y-4">
          <div className="rounded-lg border border-border bg-surface p-5 shadow-subtle">
            ...
            <p className="mt-1 text-sm text-muted-foreground">{t("import.mappingHint", { count: rows.length })}</p>
            <ul className="mt-4 space-y-3">
              {schema.fields.map((field) => (
                <li key={field.key} className="grid grid-cols-[minmax(0,1fr)_minmax(0,1fr)] items-center gap-3">
                  <span className="min-w-0 truncate text-sm font-medium">
                    {t(field.label)}
                    {field.required ? <span className="ml-1 text-danger">*</span> : null}
                  </span>
                  <Select
                    value={mapping[field.key] === null || mapping[field.key] === undefined ? NONE : String(mapping[field.key])}
                    onValueChange={(value) => setMapping({ ...mapping, [field.key]: value === NONE ? null : Number(value) })}
                  >
                    <SelectTrigger aria-label={t(field.label)}><SelectValue /></SelectTrigger>
                    <SelectContent>
                      <SelectItem value={NONE}>{t("import.notMapped")}</SelectItem>
                      {headers.map((header, index) => (
                        <SelectItem key={`${header}-${index}`} value={String(index)}>{header || `#${index + 1}`}</SelectItem>
                      ))}
                    </SelectContent>
                  </Select>
                </li>
          ...
          <div className="flex flex-wrap gap-2">
            <Button variant="ghost" onClick={() => setStep(2)}>{t("contracts.previousStep")}</Button>
      ...
      {step === 4 ? (
        <section className="space-y-4">
            ...
            <table className="w-full text-sm">
              <thead className="sticky top-0 bg-muted/80 text-left text-xs font-semibold text-muted-foreground">
                <tr>
                  ...
                  {headers.map((header, index) => <th key={`${header}-${index}`} className="h-9 px-3">{header}</th>)}
                </tr>
              </thead>
              <tbody>
                {validated.map((row) => (
                  <tr key={row.index} className={cn("border-t border-border", row.status === "error" && "bg-danger/5")}>
                    ...
                    {row.raw.map((cell, index) => (
                      <td key={index} className="max-w-48 truncate px-3 py-2">{cell}</td>
                    ))}
                  </tr>
                ))}
              </tbody>
          ...
          <div className="flex flex-wrap gap-2">
            <Button variant="ghost" onClick={() => setStep(3)}>{t("contracts.previousStep")}</Button>
            {counts.error > 0 ? (
              <Button variant="outline" onClick={downloadErrors}>
                <Download className="size-4" />{t("import.downloadErrors")}
              </Button>
            ) : null}
            <Button
      ...
      {step === 5 && summary ? (
        <section className="rounded-lg border border-success/25 bg-success/5 p-6 text-center">
          ...
          <Button className="mt-5" variant="outline" onClick={reset}>{t("import.importAnother")}</Button>
        </section>
      ) : null}
    </div>
  );
    ...
    <div className="space-y-6">
      ...
      >
        <Tabs defaultValue="units">
          <TabsList>
            {KINDS.map((kind) => <TabsTrigger key={kind} value={kind}>{t(`import.kinds.${kind}`)}</TabsTrigger>)}
          </TabsList>
          {KINDS.map((kind) => (
