...
function UnitsPage() {
  ...
  const columns: DataTableColumn<UnitContext>[] = [
    ...
    {
      key: "bathrooms",
      header: t("units.columns.bathrooms"),
      numeric: true,
      sortValue: (row) => Number(row.unit.bathrooms ?? 0),
      cell: (row) => row.unit.bathrooms ?? "—",
    },
    {
      key: "sqm",
      header: t("units.columns.sqm"),
      numeric: true,
      sortValue: (row) => Number(row.unit.sqm ?? 0),
      ...
      <div className="grid gap-3 sm:grid-cols-3">
        <Select value={propertyFilter} onValueChange={setPropertyFilter}>
          <SelectTrigger aria-label={t("units.filters.property")}>
            <SelectValue placeholder={t("units.filters.property")} />
          </SelectTrigger>
          <SelectContent>
        ...
        <Select value={statusFilter} onValueChange={setStatusFilter}>
          <SelectTrigger aria-label={t("units.filters.status")}>
            <SelectValue placeholder={t("units.filters.status")} />
          </SelectTrigger>
          <SelectContent>
        ...
        <Select value={bedroomFilter} onValueChange={setBedroomFilter}>
          <SelectTrigger aria-label={t("units.filters.bedrooms")}>
            <SelectValue placeholder={t("units.filters.bedrooms")} />
          </SelectTrigger>
          <SelectContent>
            <SelectItem value={ALL}>{t("units.filters.allBedrooms")}</SelectItem>
            {["1", "2", "3", "4"].map((count) => (
              <SelectItem key={count} value={count}>
                {t("units.filters.bedroomCount", { count: Number(count) })}
              </SelectItem>
            ))}
          </SelectContent>
        </Select>
      ...
      >
        {view === "table" ? (
          <DataTable
            ...
            searchValue={(row) =>
              `${row.unit.unit_number} ${row.property?.name ?? ""} ${row.tenant?.full_name ?? ""}`
            }
            onRowClick={(row) =>
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
            {rows.map((row) => (
              ...
              >
                ...
                <div className="mt-3">
                  <MoneyText value={Number(row.unit.base_rent)} />
                </div>
                {row.tenant ? (
      ...
      >
        ...
        <div className="grid gap-4 sm:grid-cols-3">
          <Field label={t("units.fields.bedrooms")} htmlFor="unit-bedrooms">
            <Input
              ...
              value={form.bedrooms}
              onChange={(event) =>
                setForm({ ...form, bedrooms: event.target.value.replace(/\D/g, "") })
              }
            />
          </Field>
          <Field label={t("units.fields.bathrooms")} htmlFor="unit-bathrooms">
            <Input
              ...
              onChange={(event) =>
                setForm({ ...form, bathrooms: event.target.value.replace(/[^\d.]/g, "") })
              }
            />
          </Field>
          <Field label={t("units.fields.sqm")} htmlFor="unit-sqm">
            <Input
              ...
              value={form.sqm}
              onChange={(event) =>
                setForm({ ...form, sqm: event.target.value.replace(/[^\d.]/g, "") })
              }
            />
          </Field>
        </div>
        <Field label={t("units.fields.baseRent")}>
          <MoneyInput
            ...
            onChange={(value) => setForm({ ...form, base_rent: value })}
          />
        </Field>
      </FormDialog>
    </div>
