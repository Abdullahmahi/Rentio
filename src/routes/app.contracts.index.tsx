...
function ContractsPage() {
  ...
  const columns: DataTableColumn<LeaseContext>[] = [
    ...
    { key: "balance", header: t("contracts.columns.balance"), numeric: true,
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        actions={<Button onClick={() => setWizardOpen(true)}><Plus className="size-4" />{t("contracts.new")}</Button>}
      />

      <div className="grid gap-3 sm:grid-cols-2 lg:max-w-xl">
        <Select value={statusFilter} onValueChange={setStatusFilter}>
          <SelectTrigger aria-label={t("contracts.columns.status")}><SelectValue /></SelectTrigger>
          <SelectContent>
            <SelectItem value={ALL}>{t("contracts.filters.allStatuses")}</SelectItem>
            {(["borrador", "activo", "por_vencer", "terminado", "rescindido"] as const).map((status) => (
              <SelectItem key={status} value={status}>{t(`leaseStatus.${status}`)}</SelectItem>
            ))}
          </SelectContent>
        </Select>
      ...
      >
        <DataTable
          ...
          searchValue={(row) => `${row.unit?.unit_number ?? ""} ${row.primaryTenant?.full_name ?? ""} ${row.property?.name ?? ""}`}
      ...
      {rows.some((row) => isExpiringSoon(row.lease)) ? (
        <p className="text-sm text-muted-foreground">
          {t("contracts.expiringHint", {
            ...
            days: Math.min(...rows.filter((row) => isExpiringSoon(row.lease)).map((row) => daysUntilEnd(row.lease))),
          })}
        </p>
      ) : null}
      ...
      <LeaseWizard
        ...
        onCreated={(leaseId) => void navigate({ to: "/app/contracts/$id", params: { id: leaseId } })}
      />
    </div>
  );
}
