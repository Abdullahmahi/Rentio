...
import { InvoiceStatusBadge, PaymentStatusBadge, effectiveInvoiceStatus } from "@/components/rentio/status";
...
import { logActivity, qk, useActorId, usePortfolio, useSettings, useToastMutation } from "@/lib/queries";
...
function ReceiptDetailPage() {
  ...
  const [line, setLine] = useState<{ description: string; category: Enums<"line_category">; quantity: string; amount: number | "" }>({
    description: "", category: "otro", quantity: "1", amount: "",
  });

  const invoice = useQuery({
    queryKey: qk.invoice(id),
    queryFn: async () => {
      const [{ data: head, error: headError }, { data: lines, error: linesError }, { data: balance }] = await Promise.all([
  ...
  const syncTotal = async () => {
    const { data } = await supabase.from("invoice_lines").select("amount, quantity").eq("invoice_id", id);
    const total = (data ?? []).reduce((sum, row) => sum + Number(row.amount) * Number(row.quantity), 0);
  ...
  const addLine = useToastMutation({
    ...
    onSuccess: () => { setLineOpen(false); setLine({ description: "", category: "otro", quantity: "1", amount: "" }); },
  });
  ...
  const applyLateFee = useToastMutation({
    mutationFn: async () => {
      ...
      const { error: caught } = await supabase.from("invoice_lines").insert({
        invoice_id: id, description: t("receipts.lines.lateFee"), category: "recargo", quantity: 1, amount,
      });
      if (caught) throw caught;
      await syncTotal();
  ...
  const send = useToastMutation({
    mutationFn: async () => {
      const { error: caught } = await supabase.from("invoices").update({ status: "enviado" }).eq("id", id);
      if (caught) throw caught;
      await logActivity(actorId, "invoice", id, "send", {});
    },
  ...
  const cancel = useToastMutation({
    mutationFn: async (reason: string) => {
      const { error: caught } = await supabase.from("invoices")
        .update({ status: "cancelado", cancel_reason: reason }).eq("id", id);
      if (caught) throw caught;
  ...
  const downloadPdf = async () => {
    try {
      const { data, error: caught } = await supabase.functions.invoke("generate-invoice-pdf", { body: { invoice_id: id } });
      if (caught) throw caught;
  ...
  return (
    <div className="space-y-6">
      <Link to="/app/receipts" className="inline-flex items-center gap-1.5 text-sm font-medium text-muted-foreground hover:text-foreground">
          ...
          <>
                ...
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  {isDraft ? (
                    <Button onClick={() => send.mutate(undefined)} disabled={send.isPending}>
                      <Send className="size-4" />{t("receipts.send")}
                    </Button>
                  ) : null}
                  ...
                  {!isCancelled ? (
                    <AdminOnly>
                      <Button variant="outline" onClick={() => setCancelOpen(true)}>{t("receipts.cancel")}</Button>
                    </AdminOnly>
                  ) : null}
                </div>
              }
            ...
            <section className="grid gap-6 rounded-lg border border-border bg-surface p-6 shadow-subtle sm:grid-cols-2">
              <div>
                ...
                <p className="text-sm text-muted-foreground">
                  {[settings.data?.city, settings.data?.state, settings.data?.postal_code].filter(Boolean).join(", ")}
                </p>
                {settings.data?.email ? <p className="text-sm text-muted-foreground">{settings.data.email}</p> : null}
              </div>
              <dl className="space-y-1.5 text-sm sm:text-right">
                <div><dt className="inline text-muted-foreground">{t("receipts.columns.folio")}: </dt>
                  <dd className="numeric inline font-medium">{head.invoice_number ?? "—"}</dd></div>
                <div><dt className="inline text-muted-foreground">{t("receipts.columns.period")}: </dt>
                  <dd className="inline font-medium">{formatPeriod(head.period_month, i18n.language)}</dd></div>
                <div><dt className="inline text-muted-foreground">{t("receipts.columns.issue")}: </dt>
                  <dd className="numeric inline font-medium">{formatMexicoDate(head.issue_date)}</dd></div>
                <div><dt className="inline text-muted-foreground">{t("receipts.columns.due")}: </dt>
                  <dd className="numeric inline font-medium">{formatMexicoDate(head.due_date)}</dd></div>
                <div><dt className="inline text-muted-foreground">{t("units.columns.unit")}: </dt>
                  <dd className="inline font-medium">{context?.unit?.unit_number ?? "—"} · {context?.property?.name ?? ""}</dd></div>
                <div><dt className="inline text-muted-foreground">{t("contracts.columns.tenant")}: </dt>
                  <dd className="inline font-medium">{context?.primaryTenant?.full_name ?? "—"}</dd></div>
              </dl>
            </section>
            ...
            <section className="space-y-3">
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  {isDraft ? (
                    <Button variant="outline" size="sm" onClick={() => setLineOpen(true)}>
                      <Plus className="size-4" />{t("receipts.addLine")}
                    </Button>
                  ) : null}
                </div>
              </div>
                  ...
                  <tbody>
                    {lines.map((row: Tables<"invoice_lines">) => (
                      <tr key={row.id} className="border-t border-border">
                        ...
                        <td className="px-4 py-2.5 text-muted-foreground">{t(`lineCategory.${row.category}`)}</td>
                        <td className="numeric px-4 py-2.5 text-right">{Number(row.quantity)}</td>
                        <td className="px-4 py-2.5 text-right"><MoneyText value={Number(row.amount) * Number(row.quantity)} /></td>
                        {isDraft ? (
                    ...
                    <tr className="border-t border-border bg-muted/40">
                      <td className="px-4 py-2.5 font-medium" colSpan={isDraft ? 4 : 3}>{t("receipts.subtotal")}</td>
                      <td className="px-4 py-2.5 text-right"><MoneyText value={subtotal} /></td>
                      {isDraft ? <td /> : null}
                    </tr>
                    <tr className="border-t border-border bg-muted/40">
                      <td className="px-4 py-2.5 font-semibold" colSpan={isDraft ? 4 : 3}>{t("receipts.columns.total")}</td>
                      <td className="px-4 py-2.5 text-right"><MoneyText value={subtotal} className="font-semibold" /></td>
                      {isDraft ? <td /> : null}
                    </tr>
                  </tbody>
                </table>
              ...
              <div className="grid gap-4 sm:grid-cols-3">
                {([
                  ...
                  ["receipts.columns.balance", subtotal - paid, subtotal - paid > 0 ? "text-danger" : ""],
                  ...
                  <div key={key} className="rounded-lg border border-border bg-surface p-4 shadow-subtle">
                    ...
                    <p className={`numeric mt-1 text-xl font-semibold ${tone}`}>{formatMXN(value)}</p>
                  </div>
                ))}
              </div>
            </section>
            ...
            <section className="space-y-3">
              ...
              {(payments.data?.length ?? 0) === 0 ? (
                <EmptyState message={t("receipts.noPayments")} description={t("receipts.noPaymentsDescription")} />
              ) : (
                <ul className="divide-y divide-border overflow-hidden rounded-lg border border-border bg-surface">
                  {payments.data?.map((allocation) => {
                    const payment = allocation.payments as unknown as
                      { paid_at: string; method: Enums<"payment_method">; reference: string | null; status: Enums<"payment_status"> } | null;
                    return (
                      <li key={allocation.id} className="grid grid-cols-[minmax(0,1fr)_auto] items-center gap-4 px-4 py-3">
                        <div className="min-w-0">
                          <p className="text-sm font-medium">
                            {payment ? formatMexicoDate(payment.paid_at) : "—"} · {payment ? t(`paymentMethod.${payment.method}`) : "—"}
                          </p>
                          <p className="numeric truncate text-xs text-muted-foreground">{payment?.reference ?? "—"}</p>
                        </div>
            ...
            <FormDialog
              open={lineOpen}
              onOpenChange={(next) => { setLineOpen(next); if (!next) setError(null); }}
              title={t("receipts.addLine")}
              error={error}
              pending={addLine.isPending}
              onSubmit={() => {
                setError(null);
                if (!line.description.trim()) return setError(t("receipts.errors.descriptionRequired"));
            ...
            >
              <Field label={t("receipts.line.description")} htmlFor="line-description">
                <Input id="line-description" value={line.description}
                  onChange={(event) => setLine({ ...line, description: event.target.value })} />
              </Field>
              <div className="grid gap-4 sm:grid-cols-2">
                <Field label={t("receipts.line.category")}>
                  <Select value={line.category} onValueChange={(value) => setLine({ ...line, category: value as Enums<"line_category"> })}>
                    <SelectTrigger><SelectValue /></SelectTrigger>
                    <SelectContent>
                      {LINE_CATEGORIES.map((category) => (
                        <SelectItem key={category} value={category}>{t(`lineCategory.${category}`)}</SelectItem>
                      ))}
                    </SelectContent>
                  </Select>
                </Field>
                <Field label={t("receipts.line.quantity")} htmlFor="line-quantity">
                  <Input id="line-quantity" inputMode="numeric" className="numeric" value={line.quantity}
                    onChange={(event) => setLine({ ...line, quantity: event.target.value.replace(/\D/g, "") })} />
                </Field>
              </div>
              <Field label={t("receipts.line.amount")}>
                <MoneyInput value={line.amount} onChange={(value) => setLine({ ...line, amount: value })} />
              </Field>
            </FormDialog>
            ...
            <FormDialog
              open={cancelOpen}
              onOpenChange={(next) => { setCancelOpen(next); if (!next) setError(null); }}
