...
import { InvoiceStatusBadge, LeaseStatusBadge, PaymentStatusBadge, effectiveInvoiceStatus } from "@/components/rentio/status";
...
function ContractDetailPage() {
  ...
  const [endForm, setEndForm] = useState({
    move_out_date: todayIso(), refunded: "" as number | "", retained: "" as number | "", notes: "",
  });
  ...
  const payments = useQuery({
    ...
    queryFn: async () => {
      const { data, error: caught } = await supabase
        .from("payments").select("*").eq("lease_id", id).order("paid_at", { ascending: false });
  ...
  const updateRent = useToastMutation({
    mutationFn: async (amount: number) => {
      const { error: caught } = await supabase.from("leases").update({ rent_amount: amount }).eq("id", id);
      if (caught) throw caught;
      await logActivity(actorId, "lease", id, "update_rent", { from: lease?.rent_amount, to: amount });
    },
  ...
  const terminate = useToastMutation({
    mutationFn: async (values: typeof endForm) => {
      ...
      const { error: unitError } = await supabase.from("units").update({ status: "vacante" }).eq("id", lease.unit_id);
      if (unitError) throw unitError;

      // Parking follows the lease out — otherwise it keeps getting billed.
      const { error: parkingError } = await supabase.from("parking_spaces")
        .update({ lease_id: null, status: "disponible" }).eq("lease_id", id);
      if (parkingError) throw parkingError;

      await logActivity(actorId, "lease", id, "terminate", {
        move_out_date: values.move_out_date, refunded: values.refunded, retained: values.retained,
      });
    },
  ...
  return (
    <div className="space-y-6">
      <Link to="/app/contracts" className="inline-flex items-center gap-1.5 text-sm font-medium text-muted-foreground hover:text-foreground">
        <ArrowLeft className="size-4" />{t("contracts.backToList")}
      </Link>
      ...
      <QueryState
        ...
        empty={<EmptyState icon={FileText} message={t("contracts.notFound")} description={t("contracts.notFoundDescription")} />}
      >
        {context && lease ? (
          <>
            <PageHeader
              ...
              actions={
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <Button variant="outline" onClick={() => setRenewSeed(seedFromLease(lease, portfolio.data?.leaseTenants.filter((link) => link.lease_id === id) ?? []))}>
                    <RefreshCw className="size-4" />{t("contracts.renew")}
                  </Button>
                  {isActive(lease) ? (
                    <Button variant="outline" onClick={() => {
                      setEndForm({ move_out_date: todayIso(), refunded: Number(lease.deposit_amount), retained: 0, notes: "" });
                      setEndOpen(true);
                    }}>
            ...
            <section className="grid gap-4 rounded-lg border border-border bg-surface p-5 shadow-subtle sm:grid-cols-4">
              {([
                ...
                <div key={key}>
                  ...
                  <MoneyText value={value} className={`mt-1 block text-xl font-semibold ${className}`} />
                </div>
              ))}
              <div>
                <p className="text-xs font-medium text-muted-foreground">{t("contracts.balance.oldestOverdue")}</p>
                <p className={`numeric mt-1 text-xl font-semibold ${context.balance?.oldest_overdue_date ? "text-danger" : ""}`}>
                  {context.balance?.oldest_overdue_date ? formatMexicoDate(context.balance.oldest_overdue_date) : "—"}
                </p>
              </div>
            </section>
            ...
            <Tabs defaultValue="summary">
                ...
                <div className="grid gap-4 lg:grid-cols-2">
                  <section className="rounded-lg border border-border bg-surface p-5 shadow-subtle">
                    ...
                    <dl className="mt-3">
                      ...
                      <Row label={t("contracts.columns.start")}>{formatMexicoDate(lease.start_date)}</Row>
                      <Row label={t("contracts.columns.end")}>{formatMexicoDate(lease.end_date)}</Row>
                      <Row label={t("contracts.fields.rent")}>
                        <span className="inline-flex items-center gap-2">
                          ...
                          <AdminOnly>
                            <Button size="icon" variant="ghost" aria-label={t("contracts.editRent")}
                              onClick={() => { setRentAmount(Number(lease.rent_amount)); setRentOpen(true); }}>
                      ...
                      <Row label={t("contracts.fields.lateFee")}><MoneyText value={Number(lease.late_fee_amount)} /></Row>
                    </dl>
                  </section>
                  ...
                  <section className="rounded-lg border border-border bg-surface p-5 shadow-subtle">
                    ...
                    <dl className="mt-3">
                      <Row label={t("contracts.fields.primaryTenant")}>{context.primaryTenant?.full_name ?? "—"}</Row>
                      <Row label={t("contracts.fields.coTenants")}>
                        {context.coTenants.length ? context.coTenants.map((person) => person.full_name).join(", ") : "—"}
                      </Row>
                      <Row label={t("contracts.fields.guarantors")}>
                        {context.guarantors.length ? context.guarantors.map((person) => person.full_name).join(", ") : "—"}
                      </Row>
                    </dl>
                    ...
                    <dl className="mt-3">
                      ...
                      <Row label={t("contracts.fields.depositStatus")}>
                        {t(`contracts.depositStatus.${lease.deposit_status}`, { defaultValue: lease.deposit_status })}
                      </Row>
                      {lease.move_out_date ? (
                        <>
                          <Row label={t("contracts.fields.moveOut")}>{formatMexicoDate(lease.move_out_date)}</Row>
                          <Row label={t("contracts.fields.refunded")}><MoneyText value={Number(lease.deposit_refunded ?? 0)} /></Row>
                          <Row label={t("contracts.fields.retained")}><MoneyText value={Number(lease.deposit_retained ?? 0)} /></Row>
                        </>
                      ) : null}
                    </dl>
                  </section>
              ...
              <TabsContent value="invoices" className="mt-4">
                <QueryState
                  isLoading={invoices.isLoading} error={invoices.error}
                  isEmpty={(invoices.data?.length ?? 0) === 0}
                  onRetry={() => void invoices.refetch()}
                  skeleton={<RowsSkeleton count={4} />}
                  empty={<EmptyState message={t("receipts.emptyTitle")} description={t("receipts.emptyDescription")} />}
                >
                  <DataTable
                    columns={invoiceColumns} data={invoices.data ?? []} getRowId={(row) => row.id}
              ...
              <TabsContent value="payments" className="mt-4">
                <QueryState
                  isLoading={payments.isLoading} error={payments.error}
              ...
              <TabsContent value="parking" className="mt-4">
                {context.parking.length === 0 ? (
                  <EmptyState icon={CarFront} message={t("contracts.noParking")} description={t("contracts.noParkingDescription")} />
                ) : (
            ...
            <FormDialog
              open={rentOpen}
              onOpenChange={(next) => { setRentOpen(next); if (!next) setError(null); }}
              ...
              onSubmit={() => {
                setError(null);
                if (rentAmount === "" || Number(rentAmount) <= 0) return setError(t("contracts.errors.rentRequired"));
            ...
            <FormDialog
              open={endOpen}
              onOpenChange={(next) => { setEndOpen(next); if (!next) setError(null); }}
              ...
              onSubmit={() => {
                ...
                if (total > Number(lease.deposit_amount) + 0.005) return setError(t("contracts.errors.depositOverflow"));
                terminate.mutate(endForm);
              }}
            >
              <Field label={t("contracts.fields.moveOut")} htmlFor="end-date">
                <Input id="end-date" type="date" className="numeric" value={endForm.move_out_date}
                  onChange={(event) => setEndForm({ ...endForm, move_out_date: event.target.value })} />
              </Field>
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                <Field label={t("contracts.fields.refunded")}>
                  <MoneyInput value={endForm.refunded} onChange={(value) => setEndForm({ ...endForm, refunded: value })} />
                </Field>
                <Field label={t("contracts.fields.retained")}>
                  <MoneyInput value={endForm.retained} onChange={(value) => setEndForm({ ...endForm, retained: value })} />
                </Field>
              </div>
              <Field label={t("contracts.fields.depositNotes")} htmlFor="end-notes">
                <Textarea id="end-notes" rows={3} value={endForm.notes}
            ...
            <LeaseWizard
              open={renewSeed !== null}
              onOpenChange={(next) => { if (!next) setRenewSeed(null); }}
              seed={renewSeed ?? undefined}
              onCreated={(leaseId) => { setRenewSeed(null); void navigate({ to: "/app/contracts/$id", params: { id: leaseId } }); }}
            />
          </>
        ) : null}
      </QueryState>
