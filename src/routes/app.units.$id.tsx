...
import { LeaseStatusBadge, UnitStatusBadge, WorkOrderPriorityBadge, WorkOrderStatusBadge } from "@/components/rentio/status";
...
export const Route = createFileRoute("/app/units/$id")({ component: UnitDetailPage });
...
function UnitDetailPage() {
  ...
  const { current, history } = useMemo(() => {
    ...
    return {
      current: all.find((row) => row.lease.status === "activo" || row.lease.status === "por_vencer"),
      history: all.filter((row) => row.lease.status !== "activo" && row.lease.status !== "por_vencer"),
    };
  }, [portfolio.data, id]);
  ...
  const workOrders = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("work_orders").select("*").eq("unit_id", id)
  ...
  const historyColumns: DataTableColumn<LeaseContext>[] = [
    { key: "tenant", header: t("contracts.columns.tenant"), sortValue: (row) => row.primaryTenant?.full_name ?? "",
      cell: (row) => row.primaryTenant?.full_name ?? "—" },
    { key: "start", header: t("contracts.columns.start"), sortValue: (row) => row.lease.start_date,
      cell: (row) => formatMexicoDate(row.lease.start_date) },
    { key: "end", header: t("contracts.columns.end"), sortValue: (row) => row.lease.end_date,
      cell: (row) => formatMexicoDate(row.lease.end_date) },
    { key: "rent", header: t("contracts.columns.rent"), numeric: true, sortValue: (row) => Number(row.lease.rent_amount),
      cell: (row) => <MoneyText value={Number(row.lease.rent_amount)} /> },
    { key: "status", header: t("contracts.columns.status"), sortValue: (row) => row.lease.status,
  ...
  const woColumns: DataTableColumn<Tables<"work_orders">>[] = [
    ...
    { key: "priority", header: t("maintenance.columns.priority"), sortValue: (row) => row.priority,
  ...
  return (
    <div className="space-y-6">
      <Link to="/app/units" className="inline-flex items-center gap-1.5 text-sm font-medium text-muted-foreground hover:text-foreground">
        <ArrowLeft className="size-4" />{t("units.backToList")}
      </Link>
      ...
      <QueryState
        ...
        empty={<EmptyState message={t("units.notFound")} description={t("units.notFoundDescription")} />}
      >
        {unit ? (
          <>
            <PageHeader
            ...
            <Tabs defaultValue="summary">
                ...
                <div className="grid gap-4 lg:grid-cols-2">
                  <section className="rounded-lg border border-border bg-surface p-5 shadow-subtle">
                    ...
                    <dl className="mt-3">
                      ...
                      <Row label={t("units.columns.baseRent")}><MoneyText value={Number(unit.base_rent)} /></Row>
                    </dl>
                  </section>
                  ...
                  <section className="rounded-lg border border-border bg-surface p-5 shadow-subtle">
                    ...
                    {current ? (
                      <dl className="mt-3">
                        <Row label={t("units.columns.tenant")}>{current.primaryTenant?.full_name ?? "—"}</Row>
                        <Row label={t("contracts.columns.rent")}><MoneyText value={Number(current.lease.rent_amount)} /></Row>
                        <Row label={t("contracts.columns.start")}>{formatMexicoDate(current.lease.start_date)}</Row>
                        <Row label={t("contracts.columns.end")}>{formatMexicoDate(current.lease.end_date)}</Row>
                ...
                {current ? (
                  <section className="rounded-lg border border-border bg-surface p-5 shadow-subtle">
                    <div className="flex flex-wrap items-center justify-between gap-3">
                      <div>
                        <h2 className="text-base font-semibold">{current.primaryTenant?.full_name ?? t("units.tabs.lease")}</h2>
                        <p className="mt-0.5 text-sm text-muted-foreground">
                          {formatMexicoDate(current.lease.start_date)} — {formatMexicoDate(current.lease.end_date)}
                        </p>
                      </div>
                    ...
                    <dl className="mt-4">
                      ...
                      <Row label={t("contracts.fields.deposit")}><MoneyText value={Number(current.lease.deposit_amount)} /></Row>
                      <Row label={t("contracts.columns.balance")}><MoneyText value={Number(current.balance?.balance ?? 0)} /></Row>
                    </dl>
                    <Button
                      className="mt-4" variant="outline"
                      onClick={() => void navigate({ to: "/app/contracts/$id", params: { id: current.lease.id } })}
                    >
                      {t("units.openLease")}
                    </Button>
                  </section>
              ...
              <TabsContent value="history" className="mt-4">
                {history.length === 0 ? (
                  <EmptyState icon={FileText} message={t("units.noHistory")} description={t("units.noHistoryDescription")} />
                ) : (
                  <DataTable
                    columns={historyColumns}
                    data={history}
                    getRowId={(row) => row.lease.id}
                    onRowClick={(row) => void navigate({ to: "/app/contracts/$id", params: { id: row.lease.id } })}
                  />
                )}
              </TabsContent>
            ...
            {current && current.parking.length > 0 ? (
              <p className="flex items-center gap-2 text-sm text-muted-foreground">
                ...
                {t("units.parkingAssigned", { labels: current.parking.map((space) => space.label).join(", ") })}
              </p>
            ) : null}
          </>
        ) : null}
