...
const EMPTY_FORM = {
  name: "", street: "", colonia: "", city: "Ciudad de México",
  state: "Ciudad de México", postal_code: "", notes: "",
};
...
function PropertiesPage() {
  ...
  const create = useToastMutation({
    mutationFn: async (values: typeof EMPTY_FORM) => {
      const { data, error: caught } = await supabase.from("properties").insert(values).select("id").single();
    ...
    <div className="space-y-6">
      <PageHeader
        ...
        actions={
          <Button onClick={() => setOpen(true)}>
            <Plus className="size-4" />{t("properties.new")}
          </Button>
        }
      />
      ...
      >
        <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
          {cards.map(({ property, stats }) => (
            <button
              key={property.id}
              onClick={() => void navigate({ to: "/app/properties/$id", params: { id: property.id } })}
            ...
            >
              ...
              <div
                ...
                role="img"
                aria-label={t("properties.occupancyLabel", { occupied: stats.occupied, total: stats.total })}
              >
                <div className="h-full rounded-full bg-primary" style={{ width: `${Math.round(stats.rate * 100)}%` }} />
              </div>
      ...
      >
        <Field label={t("properties.fields.name")} htmlFor="property-name">
          <Input id="property-name" value={form.name} onChange={(event) => setForm({ ...form, name: event.target.value })} />
        </Field>
        <Field label={t("properties.fields.street")} htmlFor="property-street">
          <Input id="property-street" value={form.street} onChange={(event) => setForm({ ...form, street: event.target.value })} />
        </Field>
        <div className="grid gap-4 sm:grid-cols-2">
          <Field label={t("properties.fields.colonia")} htmlFor="property-colonia">
            <Input id="property-colonia" value={form.colonia} onChange={(event) => setForm({ ...form, colonia: event.target.value })} />
          </Field>
          <Field label={t("properties.fields.postalCode")} htmlFor="property-cp">
            <Input id="property-cp" inputMode="numeric" maxLength={5} value={form.postal_code}
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <Field label={t("properties.fields.city")} htmlFor="property-city">
            <Input id="property-city" value={form.city} onChange={(event) => setForm({ ...form, city: event.target.value })} />
          </Field>
        ...
        <Field label={t("properties.fields.notes")} htmlFor="property-notes">
          <Textarea id="property-notes" rows={3} value={form.notes} onChange={(event) => setForm({ ...form, notes: event.target.value })} />
        </Field>
      </FormDialog>
    </div>
  );
