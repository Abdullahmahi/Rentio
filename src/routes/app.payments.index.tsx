...
const ALL = "__all__";
const METHODS: Enums<"payment_method">[] = ["spei", "efectivo", "deposito", "oxxo", "cheque", "tarjeta"];
...
function PaymentsPage() {
  ...
  const payments = useQuery({
    ...
    queryFn: async () => {
      const { data, error: caught } = await supabase.from("payments").select("*").order("paid_at", { ascending: false });
  ...
  const openInvoices = useQuery({
    ...
    queryFn: async () => {
      const { data, error: caught } = await supabase
        .from("invoice_balances").select("*").gt("balance", 0);
      if (caught) throw caught;
      const ids = (data ?? []).map((row) => row.invoice_id).filter((value): value is string => Boolean(value));
      if (ids.length === 0) return [] as (OpenInvoice & { leaseId: string })[];
      const { data: heads, error: headError } = await supabase
        .from("invoices").select("id, lease_id, invoice_number, due_date, status").in("id", ids);
      ...
      return (data ?? []).flatMap((row) => {
        ...
        return [{
          id: head.id, leaseId: head.lease_id, invoiceNumber: head.invoice_number,
          dueDate: head.due_date, balance: Number(row.balance ?? 0),
        }];
      });
    },
  });
  ...
  const contextByLease = useMemo(() => new Map(contexts.map((context) => [context.lease.id, context])), [contexts]);

  const decorate = (payment: Tables<"payments">): PaymentRow => {
  ...
  }, [payments.data, contextByLease, methodFilter, propertyFilter, fromDate, toDate]);
  ...
  const leaseOptions = useMemo<ComboboxOption[]>(
    () => contexts.filter((context) => isActive(context.lease)).map((context) => ({
      value: context.lease.id,
      label: leaseLabel(context, t("units.columns.unit")),
      hint: t("payments.currentBalance", { amount: formatMXN(Number(context.balance?.balance ?? 0)) }),
  ...
  const insertAllocations = async (paymentId: string, rows: { invoiceId: string; amount: number }[]) => {
    if (rows.length === 0) return;
    const { error: caught } = await supabase.from("payment_allocations").insert(
      rows.map((row) => ({ payment_id: paymentId, invoice_id: row.invoiceId, amount: row.amount })),
    );
    if (caught) throw caught;
  };
  ...
  const refreshInvoiceStatuses = async (invoiceIds: string[]) => {
    ...
    const { data } = await supabase.from("invoice_balances").select("*").in("invoice_id", invoiceIds);
    for (const row of data ?? []) {
      if (!row.invoice_id) continue;
      const paid = Number(row.paid ?? 0);
      const total = Number(row.total ?? 0);
      const status: Enums<"invoice_status"> = paid >= total - 0.005 ? "pagado" : paid > 0 ? "pagado_parcial" : "enviado";
      await supabase.from("invoices").update({ status }).eq("id", row.invoice_id)
        .neq("status", "cancelado").neq("status", "borrador");
    }
  };

  const confirm = useToastMutation({
    mutationFn: async (payment: PaymentRow) => {
      const open = (openInvoices.data ?? []).filter((invoice) => invoice.leaseId === payment.lease_id);
      const { allocations } = allocateOldestFirst(Number(payment.amount), open);

      const { error: caught } = await supabase.from("payments").update({
        status: "confirmado", confirmed_by: actorId, confirmed_at: new Date().toISOString(),
      }).eq("id", payment.id);
      if (caught) throw caught;
      ...
      await logActivity(actorId, "payment", payment.id, "confirm", {
        amount: payment.amount, allocations: allocations.length,
      });
    },
  ...
  const reject = useToastMutation({
    mutationFn: async ({ payment, why }: { payment: PaymentRow; why: string }) => {
      const { error: caught } = await supabase.from("payments")
        .update({ status: "cancelado", void_reason: why }).eq("id", payment.id);
      if (caught) throw caught;
  ...
  const voidPayment = useToastMutation({
    mutationFn: async ({ payment, why }: { payment: PaymentRow; why: string }) => {
      const { data: existing } = await supabase.from("payment_allocations")
        .select("invoice_id").eq("payment_id", payment.id);
      const touched = (existing ?? []).map((row) => row.invoice_id);

      const { error: deleteError } = await supabase.from("payment_allocations").delete().eq("payment_id", payment.id);
      ...
      await logActivity(actorId, "payment", payment.id, "void", { reason: why, amount: payment.amount });
    },
    successKey: "payments.voided",
    invalidate: [qk.payments, ["open-invoices"], ["invoices"], qk.portfolio],
    onSuccess: () => { setVoidFor(null); setReason(""); },
  });
  ...
  const record = useToastMutation({
    ...
    onSuccess: () => {
      setRecordOpen(false);
      setForm({ lease_id: null, amount: "", paid_at: todayIso(), method: "spei", reference: "", notes: "", receipt: null });
      setOverrides({});
    },
  });
  ...
  const historyColumns: DataTableColumn<PaymentRow>[] = [
    ...
    { key: "tenant", header: t("contracts.columns.tenant"), sortValue: (row) => row.tenantName,
      cell: (row) => <span className={row.status === "cancelado" ? "line-through text-muted-foreground" : ""}>{row.tenantName}</span> },
    ...
    { key: "amount", header: t("payments.columns.amount"), numeric: true, sortValue: (row) => Number(row.amount),
      cell: (row) => <MoneyText value={Number(row.amount)} className={row.status === "cancelado" ? "line-through" : ""} /> },
    ...
    { key: "recorded", header: t("payments.columns.recordedBy"), sortValue: (row) => row.recorded_by ?? "",
      cell: (row) => nameOf(row.recorded_by) },
    { key: "actions", header: "", cell: (row) => (
      row.status === "confirmado" ? (
        <AdminOnly>
          <Button size="sm" variant="ghost" onClick={(event) => { event.stopPropagation(); setVoidFor(row); setReason(""); }}>
            {t("payments.void")}
          </Button>
        </AdminOnly>
      ) : null
  ...
  const exportCsv = () => {
    downloadCsv(
      `pagos-${todayIso()}`,
      [t("payments.columns.date"), t("contracts.columns.tenant"), t("units.columns.unit"),
       t("payments.columns.amount"), t("payments.columns.method"), t("payments.columns.reference"),
       t("payments.columns.status"), t("payments.columns.recordedBy")],
      history.map((row) => [
        row.paid_at, row.tenantName, row.unitNumber, Number(row.amount),
        t(`paymentMethod.${row.method}`), row.reference ?? "", t(`paymentStatus.${row.status}`), nameOf(row.recorded_by),
      ]),
    );
  };
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        actions={<Button onClick={() => setRecordOpen(true)}><Plus className="size-4" />{t("payments.record")}</Button>}
      />
      ...
      <Tabs value={tab} onValueChange={setTab}>
        ...
        <TabsContent value="queue" className="mt-4">
          <QueryState
            ...
            empty={<EmptyState icon={Check} message={t("payments.queueEmptyTitle")} description={t("payments.queueEmptyDescription")} />}
          >
            <ul className="grid gap-4 lg:grid-cols-2">
              {queue.map((payment) => (
                <li key={payment.id} className="rounded-lg border border-warning/30 bg-surface p-5 shadow-subtle">
                    ...
                    <div className="min-w-0">
                      ...
                      <dl className="mt-3 space-y-1 text-sm">
                        <div className="flex gap-2">
                          ...
                          <dd className="numeric font-medium">{formatMexicoDate(payment.paid_at)}</dd>
                        </div>
                        <div className="flex gap-2">
                        ...
                        <div className="flex min-w-0 gap-2">
                          <dt className="shrink-0 text-muted-foreground">{t("payments.columns.reference")}:</dt>
                          <dd className="numeric truncate font-medium">{payment.reference ?? "—"}</dd>
                        </div>
                      </dl>
                    </div>
                    <div className="text-right">
                      <MoneyText value={Number(payment.amount)} className="block text-xl font-semibold" />
                      {payment.receipt_url ? (
                        <button
                        ...
                        <p className="mt-3 text-xs text-muted-foreground">{t("payments.noReceipt")}</p>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="mt-4 flex gap-2">
                    <Button className="flex-1" disabled={confirm.isPending} onClick={() => confirm.mutate(payment)}>
                      <Check className="size-4" />{t("payments.confirm")}
                    </Button>
                    <Button className="flex-1" variant="outline" onClick={() => { setRejectFor(payment); setReason(""); }}>
        ...
        <TabsContent value="history" className="mt-4 space-y-4">
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
            <Select value={methodFilter} onValueChange={setMethodFilter}>
              <SelectTrigger aria-label={t("payments.columns.method")}><SelectValue /></SelectTrigger>
              <SelectContent>
                <SelectItem value={ALL}>{t("payments.filters.allMethods")}</SelectItem>
                {METHODS.map((method) => <SelectItem key={method} value={method}>{t(`paymentMethod.${method}`)}</SelectItem>)}
              </SelectContent>
            </Select>
            ...
            <Input type="date" className="numeric" aria-label={t("payments.filters.from")} value={fromDate}
              onChange={(event) => setFromDate(event.target.value)} />
            <Input type="date" className="numeric" aria-label={t("payments.filters.to")} value={toDate}
          ...
          <QueryState
            ...
            empty={<EmptyState icon={WalletCards} message={t("payments.emptyTitle")} description={t("payments.emptyDescription")} />}
          >
            <DataTable
      ...
      <FormDialog
        open={recordOpen}
        onOpenChange={(next) => { setRecordOpen(next); if (!next) { setError(null); setOverrides({}); } }}
        wide
        ...
        onSubmit={() => {
          ...
          if (allocatedTotal > Number(form.amount) + 0.005) return setError(t("payments.errors.overAllocated"));
      ...
      >
        <Field label={t("parking.fields.lease")}>
          <Combobox
            ...
            value={form.lease_id}
            onChange={(value) => { setForm({ ...form, lease_id: value }); setOverrides({}); }}
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <Field label={t("payments.columns.amount")}>
            <MoneyInput value={form.amount} onChange={(value) => { setForm({ ...form, amount: value }); setOverrides({}); }} />
          </Field>
          <Field label={t("payments.fields.paidAt")} htmlFor="payment-date">
            <Input id="payment-date" type="date" className="numeric" value={form.paid_at}
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <Field label={t("payments.columns.method")}>
            <Select value={form.method} onValueChange={(value) => setForm({ ...form, method: value as Enums<"payment_method"> })}>
          ...
          <Field label={t("payments.columns.reference")} htmlFor="payment-reference">
            <Input id="payment-reference" className="numeric" value={form.reference}
            ...
            {invoicesForLease.length === 0 ? (
              ...
              <ul className="divide-y divide-border overflow-hidden rounded-lg border border-border">
                {invoicesForLease.map((invoice) => {
                  const value = overrides[invoice.id]
                    ?? suggested.allocations.find((row) => row.invoiceId === invoice.id)?.amount
                    ?? 0;
                  return (
                    <li key={invoice.id} className="grid grid-cols-[minmax(0,1fr)_10rem] items-center gap-3 px-3 py-2.5">
                      <div className="min-w-0">
                        <p className="truncate text-sm font-medium">{invoice.invoiceNumber ?? "—"}</p>
                        <p className="numeric text-xs text-muted-foreground">
                          {t("receipts.columns.due")} {formatMexicoDate(invoice.dueDate)} · {t("receipts.columns.balance")} {formatMXN(invoice.balance)}
                        </p>
                      </div>
                      <MoneyInput
                        value={value === 0 ? "" : value}
                        onChange={(next) => setOverrides({ ...overrides, [invoice.id]: next === "" ? 0 : next })}
                      />
                    </li>
                  );
                })}
      ...
      <FormDialog
        open={rejectFor !== null}
        onOpenChange={(next) => { if (!next) { setRejectFor(null); setError(null); } }}
      ...
      >
        <Field label={t("payments.fields.reason")} htmlFor="reject-reason">
          <Textarea id="reject-reason" rows={3} value={reason} onChange={(event) => setReason(event.target.value)} />
        </Field>
      </FormDialog>
      ...
      <FormDialog
        open={voidFor !== null}
        onOpenChange={(next) => { if (!next) { setVoidFor(null); setError(null); } }}
      ...
      <Dialog open={lightbox !== null} onOpenChange={(next) => { if (!next) setLightbox(null); }}>
        <DialogContent className="sm:max-w-2xl">
          <DialogHeader><DialogTitle>{t("payments.viewReceipt")}</DialogTitle></DialogHeader>
          {lightbox ? (
            <img src={lightbox} alt={t("payments.viewReceipt")} className="max-h-[70vh] w-full rounded-lg object-contain" />
          ) : null}
        </DialogContent>
      </Dialog>
    </div>
