...
async function main() {
  ...
  const demo: DemoUser[] = [
    ...
    { email: "gerente@rentio.mx", fullName: "Diego Lozano Ibarra", role: "manager", tenantId: null },
    ...(tenants ?? []).map((tenant) => ({
  ...
  const existing = new Map((await listAllUsers()).map((user) => [user.email?.toLowerCase(), user.id]));
  ...
  for (const user of demo) {
    ...
    const { error: profileError } = await admin.from("profiles").upsert({
      id, full_name: user.fullName, role: user.role, tenant_id: user.tenantId, locale: "es-MX",
    });
    if (profileError) {
    ...
    const { error: signInError } = await probe.auth.signInWithPassword({ email: user.email, password });
    console.log(`  ${user.email.padEnd(30)} ${user.role.padEnd(8)} ${signInError ? `SIGN-IN FAILED: ${signInError.message}` : "sign-in OK"}`);
  }
}
